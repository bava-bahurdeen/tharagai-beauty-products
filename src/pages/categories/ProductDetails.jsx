      ...
      <div className="bg-primary p-0 md:p-12 overflow-hidden">
        <section className="container md:px-20 md:py-12 mx-auto lg:w-[1200px] lg:h-[1340px] bg-white px-16 py-10  ">
          ...
          <div className="flex gap-2 sm:gap-5">
            <button onClick={()=>{Settoggle(1)}} className={`${toggle==1?"border-b-2 border-black text-[13px] sm:text-[20px] ":"border-none"}`}>Description</button>
            <button onClick={()=>{Settoggle(2)}} className={`${toggle==2?"  border-b-2 border-black text-[13px] sm:text-[20px] ":"border-none hidden sm:block"}`}>Addition Information</button>
            <button onClick={()=>{Settoggle(3)}} className={`${toggle==3?"border-b-2 border-black text-[13px] sm:text-[20px] ":"border-none"}`}>Reviews</button>
          </div>
          <div>
            <p className={toggle===1?"block font-[15px] mt-8 text-sm ":"hidden"} >
...
<h1 className="font-lexend  mt-8  sm:mt-6 sm:text-[40px] text-[20px]  font-medium">Related products</h1>
       {product.map((items)=>{
